...
export default function BlogCard( {post, isDarkBackground}: BlogCardProps) {
  return (
     ...
     />
     <CardTitle className='text-xl font-normal mt-4 text-center'>{post.title || "No Title"} </CardTitle>
     <br />
     <CardContent className='text-center'>
      <p>{post.description}</p> </CardContent>
      <div className='flex flex-col items-center mt-4'>
        <p className={`text-sm mb-2 ${isDarkBackground ? 'text-slate-400' : 'text-slate-600'}`}>
        Writing by Amber {new Date(post.date).toLocaleDateString() || "Unknown Date"} 
        </p>
        <a 
        href={`/posts/${post.id}`}
